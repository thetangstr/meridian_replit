...
export class MemStorage implements IStorage {
  ...
  private initializeSampleData() {
    ...
    // Create CUJs based on provided data
    const navMainCuj = this.createCuj({
      categoryId: navigationCategory.id,
      name: "Main Navigation Functions",
      description: "Core navigation features and route management"
    });
    ...
    const commCuj = this.createCuj({
      categoryId: communicationCategory.id,
      name: "Phone Integration",
      description: "Bluetooth phone features"
    });
    
    const poiCuj = this.createCuj({
      categoryId: navigationCategory.id,
      name: "Points of Interest",
      description: "Finding and navigating to points of interest"
    });
    
    const radioMediaCuj = this.createCuj({
      categoryId: mediaCategory.id,
      name: "Radio Functions",
      description: "Radio and station management"
    });
    
    const voiceAssistantCuj = this.createCuj({
      categoryId: communicationCategory.id,
      name: "Voice Assistant",
      description: "Voice command and assistant features"
    });
    
    const trafficNavCuj = this.createCuj({
      categoryId: navigationCategory.id,
      name: "Traffic and Routes",
      description: "Traffic information and route options"
    });
    
    const streamingMediaCuj = this.createCuj({
      categoryId: mediaCategory.id,
      name: "Streaming Services",
      description: "Built-in streaming services integration"
    });
    
    const messagingCuj = this.createCuj({
      categoryId: communicationCategory.id,
      name: "Messaging",
      description: "SMS and messaging features"
    });
    
    const systemCuj = this.createCuj({
      categoryId: generalCategory.id,
      name: "System Controls",
      description: "Basic system functionality and settings"
    });
    
    const offlineMapsCuj = this.createCuj({
      categoryId: navigationCategory.id,
      name: "Offline Navigation",
      description: "Offline map functionality"
    });
    
    const rearEntertainmentCuj = this.createCuj({
      categoryId: mediaCategory.id,
      name: "Rear Entertainment",
      description: "Rear seat entertainment controls"
    });

    // Create sample tasks from the provided data
    // Navigation category tasks
    this.createTask({
      cujId: navMainCuj.id,
    ...
    this.createTask({
      cujId: mediaCuj.id,
      name: "Control music playback using steering wheel controls",
    ...
    this.createTask({
      ...
      prerequisites: "Smartphone is connected via Bluetooth, Pre-defined messages are configured",
    ...
    this.createTask({
      ...
      prerequisites: "Smartphone is connected via Bluetooth",
    ...
    this.createTask({
      cujId: poiCuj.id,
      name: "Search for nearby Points of Interest",
    ...
    this.createTask({
      cujId: voiceAssistantCuj.id,
      name: "Initiate a voice assistant command",
    ...
    // Traffic navigation tasks
    this.createTask({
      cujId: trafficNavCuj.id,
    ...
    // Messaging tasks
    this.createTask({
      cujId: messagingCuj.id,
      name: "Receive and view SMS messages",
      prerequisites: "Smartphone is connected via Bluetooth, SMS access is granted",
    ...
    // Rear entertainment tasks
    this.createTask({
      cujId: rearEntertainmentCuj.id,
      name: "Control playback of rear-seat entertainment",
  ...
  async syncCujData(): Promise<{ success: boolean, message: string }> {
    try {
      // Reset CUJ-related data
      this.cujCategories = new Map<number, CujCategory>();
      this.cujs = new Map<number, Cuj>();
      this.tasks = new Map<number, Task>();
      
      // Reset ID counters for new data
      this.categoryIdCounter = 1;
      this.cujIdCounter = 1;
      this.taskIdCounter = 1;
      
      // Create fresh categories
      const navigationCategory = await this.createCujCategory({ 
        name: "Navigation", 
        description: "All navigation related functions", 
        icon: "map" 
      });
      
      const mediaCategory = await this.createCujCategory({ 
        name: "Media", 
        description: "Audio, video and entertainment functions", 
        icon: "headphones" 
      });
      
      const communicationCategory = await this.createCujCategory({ 
        name: "Communications", 
        description: "Phone, messaging and voice assistant features", 
        icon: "phone" 
      });
      
      const generalCategory = await this.createCujCategory({ 
        name: "General", 
        description: "System-wide settings and features", 
        icon: "settings" 
      });
      ...
      // Create CUJs based on provided data
      const navMainCuj = await this.createCuj({
        categoryId: navigationCategory.id,
        name: "Main Navigation Functions",
        description: "Core navigation features and route management"
      });
      
      const mediaCuj = await this.createCuj({
        categoryId: mediaCategory.id,
        name: "Media Control",
        description: "Audio playback and control"
      });
      
      const commCuj = await this.createCuj({
        categoryId: communicationCategory.id,
        name: "Phone Integration",
        description: "Bluetooth phone features"
      });
      
      const poiCuj = await this.createCuj({
        categoryId: navigationCategory.id,
        name: "Points of Interest",
        description: "Finding and navigating to points of interest"
      });
      
      const radioMediaCuj = await this.createCuj({
        categoryId: mediaCategory.id,
        name: "Radio Functions",
        description: "Radio and station management"
      });
      
      const voiceAssistantCuj = await this.createCuj({
        categoryId: communicationCategory.id,
        name: "Voice Assistant",
        description: "Voice command and assistant features"
      });
      
      const trafficNavCuj = await this.createCuj({
        categoryId: navigationCategory.id,
        name: "Traffic and Routes",
        description: "Traffic information and route options"
      });
      
      const streamingMediaCuj = await this.createCuj({
        categoryId: mediaCategory.id,
        name: "Streaming Services",
        description: "Built-in streaming services integration"
      });
      
      const messagingCuj = await this.createCuj({
        categoryId: communicationCategory.id,
        name: "Messaging",
        description: "SMS and messaging features"
      });
      
      const systemCuj = await this.createCuj({
        categoryId: generalCategory.id,
        name: "System Controls",
        description: "Basic system functionality and settings"
      });
      
      const offlineMapsCuj = await this.createCuj({
        categoryId: navigationCategory.id,
        name: "Offline Navigation",
        description: "Offline map functionality"
      });
      
      const rearEntertainmentCuj = await this.createCuj({
        categoryId: mediaCategory.id,
        name: "Rear Entertainment",
        description: "Rear seat entertainment controls"
      });
      
      // Create fresh tasks from the provided data
      // Navigation category tasks
